...
支持条件分支、加签/转交、抄送。现有资产可复用：部门树解析逐级上报、`systemRole` 做角色节点、
`overtime_ledger` / `punch_records` 的写回钩子改成"节点回调"。**这是把"能审四类"变成"能审一切"的那一步。**
...
#### N5. AI 从"能问"到"能办" ⭐⭐⭐
阶段二：自然语言 → 发起审批单（复用 N2 的模板）；阶段三：每周一自动汇总"人事异动 + 考勤概览"推送给 HR。
**依赖 N2**，先有可被程序化调用的流程定义，AI 才谈得上代办。
...
- **R11 报修服务台** ⭐⭐：`tickets` + SLA，复用 N2 引擎后增量很小。
...
现在真正卡住它的是三件事——**考勤数据还是手工导入的（N1）、审批流程是写死的（N2）、员工看不到自己的东西（N3）**。
