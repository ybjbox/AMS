...
export default function Departments() {
              ...
              <li key={node.id} className="relative">
                ...
                >
                  <div className="flex items-center space-x-2">
                    ...
                    <span
                      className={`text-sm ${level === 0 ? 'font-semibold text-slate-800' : 'font-medium text-slate-700'}`}
                    >
                      {node.name}
                    </span>
  ...
  const renderRoleTree = (nodes: DepartmentNode[], level = 0): React.ReactNode => {
      return (
        <ul
          className={`space-y-2 ${level > 0 ? 'ml-4 border-l border-slate-200 dark:border-slate-700 pl-2 mt-2' : ''}`}
        >
          {nodes.map((node) => {
            ...
            return (
              <li key={node.id} className="relative">
                <div
                  className={`border border-slate-200 dark:border-slate-700 rounded-lg overflow-hidden ${level === 0 ? 'bg-white dark:bg-slate-800' : 'bg-slate-50/50 dark:bg-slate-800/30'}`}
                >
                  <div
                  ...
                  {isExpanded && (
                    <div className="p-2 bg-white dark:bg-slate-800 border-t border-slate-100 dark:border-slate-700">
                      {deptRoles.length > 0 ? (
                        <ul className="space-y-1">
                          {deptRoles.map((role) => (
                            <li
                              key={role.id}
                              className="flex items-center justify-between p-2 rounded-md hover:bg-slate-50 dark:hover:bg-slate-700/50 group"
                            >
                      ...
                      {hasChildren && <div className="mt-2">{renderRoleTree(node.children!, level + 1)}</div>}
                    </div>
                  )}
                </div>
              </li>
