...
import { useDepartments, flattenDepartments } from '../store/useDepartmentStore';
...
export default function Departments() {
  ...
  const renderRoleTree = (nodes: DepartmentNode[], level = 0): React.ReactNode => {
      return (
        <ul
          className={`space-y-2 ${level > 0 ? 'ml-4 border-l border-zinc-200 dark:border-zinc-700 pl-2 mt-2' : ''}`}
        >
          {nodes.map((node) => (
            <RoleTreeNode
              key={node.id}
        ...
        <div className="lg:col-span-2 xl:col-span-1 bg-white dark:bg-zinc-800 shadow-sm border border-zinc-200/60 dark:border-zinc-700/60 rounded-xl flex flex-col min-h-0">
          <div className="p-4 border-b border-zinc-200 dark:border-zinc-700 flex items-center justify-between bg-zinc-50/50 dark:bg-zinc-800/50 rounded-t-xl">
            <div className="flex items-center space-x-2">
              ...
              <h2 className="text-base font-medium text-zinc-800 dark:text-zinc-200">职位设置 ({roles.length})</h2>
            </div>
            {departments.length > 0 && (
              <button
