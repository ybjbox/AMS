...
export function FolderTree({
  ...
  const renderFolderTree = (parentId: string | null, level: number = 0): React.ReactNode => {
      ...
      return (
        <ul
          className={`space-y-0.5 ${level > 0 ? 'ml-3 border-l border-zinc-200 dark:border-zinc-700 pl-1.5 mt-0.5' : ''}`}
        >
          {childFolders.map((folder) => {
            ...
            return (
              <li key={folder.id}>
                <div
                  className={`flex items-center justify-between py-1.5 px-2 rounded-md cursor-pointer transition-colors group ${isSelected ? 'bg-brand-50 dark:bg-brand-900/30 text-brand-700 dark:text-brand-400' : 'text-zinc-700 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-700'}`}
  ...
  return (
    <div className="w-full md:w-64 bg-white dark:bg-zinc-800 shadow-sm border border-zinc-200/60 dark:border-zinc-700/60 rounded-xl flex flex-col overflow-hidden shrink-0">
      <div className="p-4 border-b border-zinc-100 dark:border-zinc-700 flex justify-between items-center bg-zinc-50/50 dark:bg-zinc-800/50">
        <span className="font-medium text-zinc-700 dark:text-zinc-200 flex items-center">
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-1">
        <div
          className={`flex items-center py-1.5 px-2 rounded-md cursor-pointer transition-colors ${currentFolderId === null ? 'bg-brand-50 dark:bg-brand-900/30 text-brand-700 dark:text-brand-400' : 'text-zinc-700 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-700'}`}
          onClick={() => onSetCurrentFolderClick(null)}
        >
          <div className="w-3.5 h-3.5 mr-1.5" />
          <FolderOpen
            className={`w-4 h-4 mr-1.5 shrink-0 ${currentFolderId === null ? 'text-brand-600' : 'text-zinc-400'}`}
          />
