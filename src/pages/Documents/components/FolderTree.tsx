...
export function FolderTree({
  ...
  const renderFolderTree = (parentId: string | null, level: number = 0): React.ReactNode => {
      ...
      return (
        <ul
          className={`space-y-0.5 ${level > 0 ? 'ml-3 border-l border-slate-200 dark:border-slate-700 pl-1.5 mt-0.5' : ''}`}
        >
    ...
    <div className="w-full md:w-64 bg-white dark:bg-slate-800 shadow-sm border border-slate-200/60 dark:border-slate-700/60 rounded-xl flex flex-col overflow-hidden shrink-0">
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-1">
        ...
        >
          ...
          <FolderOpen
            className={`w-4 h-4 mr-1.5 shrink-0 ${currentFolderId === null ? 'text-blue-600' : 'text-slate-400'}`}
          />
