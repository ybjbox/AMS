    ...
    <BaseModal
      ...
      footer={
        <>
          ...
          <button type="submit" form="set-form" className="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-gradient-to-b from-blue-600 to-blue-700 shadow-inner text-base font-medium text-white hover:from-blue-500 hover:to-blue-600 active:scale-95 transition-transform sm:ml-0 sm:w-auto sm:text-sm">保存</button>
        </>
      }
    >
                  ...
                  <div key={id} className="flex flex-col sm:flex-row sm:items-center justify-between p-2 bg-slate-50 dark:bg-slate-800/50 rounded-md border border-slate-200 dark:border-slate-700 gap-2">
                    ...
                    <div className="flex items-center space-x-3 shrink-0">
                      <div className="flex items-center space-x-1 bg-white dark:bg-slate-700 rounded border border-slate-200 dark:border-slate-600 p-0.5">
                        <button type="button" onClick={() => onSetColorClick(id, false)} className={`px-2 py-1 text-xs rounded ${!settings.color ? 'bg-slate-200 dark:bg-slate-600 text-slate-800 dark:text-white' : 'text-slate-500 dark:text-slate-400'}`}>黑白</button>
                        <button type="button" onClick={() => onSetColorClick(id, true)} className={`px-2 py-1 text-xs rounded ${settings.color ? 'bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300' : 'text-slate-500 dark:text-slate-400'}`}>彩色</button>
                      </div>
                      <div className="flex items-center space-x-1 bg-white dark:bg-slate-700 rounded border border-slate-200 dark:border-slate-600 p-0.5">
                        <button type="button" onClick={() => onSetDuplexClick(id, false)} className={`px-2 py-1 text-xs rounded ${!settings.duplex ? 'bg-slate-200 dark:bg-slate-600 text-slate-800 dark:text-white' : 'text-slate-500 dark:text-slate-400'}`}>单面</button>
                        <button type="button" onClick={() => onSetDuplexClick(id, true)} className={`px-2 py-1 text-xs rounded ${settings.duplex ? 'bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300' : 'text-slate-500 dark:text-slate-400'}`}>双面</button>
                      </div>
                      <div className="flex items-center bg-white dark:bg-slate-700 rounded border border-slate-200 dark:border-slate-600">
                        <button type="button" onClick={() => onSetCopiesClick(id, 'dec')} className="px-2 py-1 text-slate-500 hover:text-slate-700 dark:hover:text-slate-300">-</button>
                        <span className="text-xs w-6 text-center text-slate-700 dark:text-slate-300">{settings.copies}</span>
                        <button type="button" onClick={() => onSetCopiesClick(id, 'inc')} className="px-2 py-1 text-slate-500 hover:text-slate-700 dark:hover:text-slate-300">+</button>
                      </div>
                    </div>
                  </div>
                );
