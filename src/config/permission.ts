 ...
 *   - attendance:purge  →  整表清空排班/打卡记录 = ADMIN（DELETE /attendance/schedules 与 /records）
 ...
 *   - departments:view  →  部门管理页（顶层 /departments）读
 *   - departments:manage →  部门树/职位写操作 = ADMIN（authMiddleware 对 /departments 的 POST|PUT|PATCH|DELETE 定 ADMIN）；
 *                           刻意不授予 HR，否则 HR 看得到写按钮却必然 403
 ...
 *   - forms:view        →  /api/form/* 策略 = EMPLOYEE（业务单正文润色，同上按 aiConfig 二次判断）
...
export const permissions: Record<string, string[]> = {
  ...
  [SystemRole.HR]: [
    ...
    'forms:view',
    'settings:view',
    // attendance:purge（整表清空排班/打卡记录）刻意不授予 HR：
    // 服务端 DELETE /attendance/schedules 与 /records 均为 requireRole("ADMIN")。
  ],
  [SystemRole.EMPLOYEE]: [
    'dashboard:view',
    'attendance:view',
