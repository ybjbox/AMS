 ...
 * `space-y-8 animate-in fade-in duration-400`）；全高滚动页追加 `flex-1 min-h-0`。
 */
export function PageContainer({ children, className = '', width = '7xl' }: PageContainerProps) {
  return (
    <div
      className={`w-full min-h-full mx-auto ${WIDTH_CLASS[width]} px-3 sm:px-4 lg:px-6 pt-3 sm:pt-4 lg:pt-6 pb-6 print:pb-8 flex flex-col ${className}`}
    >
      {children}
    </div>
  );
