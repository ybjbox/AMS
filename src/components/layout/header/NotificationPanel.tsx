        ...
        <div className="flex space-x-2">
          {unreadCount > 0 && (
            <button
              ...
              className="text-xs text-brand-600 hover:text-brand-700 dark:text-brand-400 dark:hover:text-brand-300 flex items-center"
            >
        ...
        {notifications.length === 0 ? (
          ...
          <div className="divide-y divide-zinc-100 dark:divide-zinc-700">
            {notifications.map((notification) => (
               <div
                key={notification.id}
                className={`relative p-4 pl-5 hover:bg-zinc-50 dark:hover:bg-zinc-700/50 transition-colors cursor-pointer ${!notification.read ? 'bg-brand-50/50 dark:bg-brand-900/20' : ''}`}
                onClick={() => handleMarkAsRead(notification.id)}
              >
                <span className={`absolute left-0 top-2 bottom-2 w-0.5 rounded-full ${
                  notification.type === 'error'   ? 'bg-red-500' :
                  notification.type === 'warning' ? 'bg-amber-500' :
                  notification.type === 'success' ? 'bg-brand-500' :
                  'bg-brand-500'
                }`} />
