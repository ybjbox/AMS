  ...
  return (
    ...
    >
       ...
       <div className={`h-20 flex items-center transition-all duration-300 ${isCollapsed ? 'justify-center px-0' : 'px-6'}`}>
         ...
         {!isCollapsed && <span className="text-lg font-bold text-zinc-800 dark:text-white ml-3 whitespace-nowrap overflow-hidden animate-in fade-in duration-300">AMS 系统</span>}
       </div>
       {/* Nav */}
       <nav className="px-3 pb-4 space-y-1.5 flex-1 overflow-y-auto overflow-x-hidden">
         {visibleNav.map((item) => {
           ...
           return (
             ...
             >
               <item.icon className={`h-5 w-5 shrink-0 transition-colors ${isActive ? 'text-blue-600 dark:text-blue-400' : 'text-zinc-400 dark:text-zinc-500 group-hover:text-zinc-600 dark:group-hover:text-zinc-300'} ${isCollapsed ? '' : 'mr-3'}`} />
               {!isCollapsed && <span className="whitespace-nowrap text-sm animate-in fade-in duration-300">{item.name}</span>}
             </Link>
           )
         })}
       </nav>
